...
- [ ] **Step 5: Update tests + fix flip fallout.** Push and read the `Rusty Fish Tests` run. Expected work:
  - The `engine-uci` test `eval_file_loads_a_network_and_keeps_it_on_error` (main.rs:453) now goes through `eval_file`/`recompute_nnue`; update it if it inspected `state.nnue` directly, and add coverage: default `EngineState` has `nnue.is_some()` (bundled), `UseNNUE false` → `nnue.is_none()`, `EvalFile <path>` then `UseNNUE true` keeps the custom net.
  - Add an `engine-search` test: `Searcher::default().has_nnue()` is `true`, and a short search on the start position returns a legal best move with a finite score.
  - **Fix any workspace test that broke from the flip.** Mate-finding / best-move tests should still pass (NNUE + `evaluate_terminal` still detects mates). Tests asserting *hand-crafted-specific score magnitudes* through a default `Searcher` will change — adjust them to construct a hand-crafted searcher (`set_nnue(None)`) if they are specifically testing the hand-crafted eval, or re-bake the expected value if they are testing search behaviour. Do NOT blindly delete assertions; decide per test whether it means to test hand-crafted eval or search.
