...
- [ ] **Step 1 (vector projection, TDD):** Fix **`N = 18`** (4 piece values × mg/eg = 8, 4 mobility × mg/eg = 8, `bishop_pair`, `passed_pawn_base`) as a `const EVAL_DIMENSIONS: usize = 18;` next to the specs so the table and vector lengths can't drift. Add `EVAL_SPSA_SPECS: [SpsaSpec; EVAL_DIMENSIONS]` (one spec per weight — e.g. piece values default±120 step 12, mobility 0..12 step 1, bishop_pair 0..80 step 6, passed_pawn 0..60 step 6) and `eval_params_to_vector(&EvalParams) -> [f64; EVAL_DIMENSIONS]` / `vector_to_eval_params(&[f64; EVAL_DIMENSIONS]) -> EvalParams` (reading `TaperedScore.middlegame`/`.endgame`, now public). Test: round-trip `EvalParams::default()`, and that `vector_to_eval_params` clamps out-of-range. Push red (functions missing) → implement → green. **The vector order must be fixed and documented** — the `eval-gate-file`/`spsa-eval` TSV interchange depends on it.
...
- [ ] **Step 1 (TDD):** A short in-process `run_eval_spsa_campaign(positions, initial: EvalParams, config) -> EvalSpsaReport` mirroring `run_spsa_campaign` but over the eval vector + `EVAL_SPSA_SPECS`, playing `theta+` vs `theta-` via the per-side plumbing from Task 3, mobility enabled on both sides. Reuse the generalized `spsa_update`/`perturb`/`SpsaRng` (they return `Vec<f64>`; use an 18-wide `to_array` analog of Task 1's helper before `vector_to_eval_params`, or have `vector_to_eval_params` accept a slice). Smoke test (2 iterations, few positions, depth-2/short) returns in-bounds `EvalParams`. Push red → implement → green.
