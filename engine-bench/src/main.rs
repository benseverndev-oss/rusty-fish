...
use engine_bench::dataset::{
    DatasetManifest, PositionRecord, TEST_SPLIT, TRAIN_SPLIT, VALIDATION_SPLIT, canonical_fen,
    deduplicate_and_split, sha256_hex, write_manifest,
};
use engine_bench::stockfish::{StockfishConfig, calibrate_node_budget, label_positions};
use engine_bench::train::{TrainConfig, generate_training_samples, train_nnue};
use engine_bench::{
    DEFAULT_TACTICAL_SUITE, ExternalMatchConfig, MatchConfig, MatchScore, SprtConfig, SpsaConfig,
    ...
    run_spsa_campaign, run_tactical_suite, sprt, sprt_tsv_report, spsa_tsv_report, summarize,
    tactical_tsv_report, throughput_tsv_report,
};
use engine_core::Board;
use engine_search::{FeatureSchema, Nnue, SearchParams};
...
fn main() -> Result<(), String> {
    ...
    if std::env::args().nth(1).as_deref() == Some("gauntlet") {
        ...
        print!(
            "{}",
            sprt_tsv_report(summarize(&records), SprtConfig::default())
        );
        return Ok(());
    }
    ...
    if std::env::args().nth(1).as_deref() == Some("spsa") {
        ...
        if let Some(iterations) = std::env::args()
            .nth(2)
    ...
    if std::env::args().nth(1).as_deref() == Some("train") {
        ...
        if let Some(epochs) = std::env::args()
            .nth(5)
            .and_then(|arg| arg.parse::<usize>().ok())
        {
            config.epochs = epochs.max(1);
        }
        if let Some(learning_rate) = std::env::args()
            .nth(6)
    ...
    if std::env::args().nth(1).as_deref() == Some("gen-data") {
        // gen-data <plies> <label_depth> <seed> [schema]: emit labelled samples as TSV.
        let plies = arg_u32(2).unwrap_or(48);
        let label_depth = arg_u32(3)
            .and_then(|d| u8::try_from(d).ok())
            .filter(|d| *d > 0);
        let seed = arg_u64(4).unwrap_or(1);
        let schema = feature_schema_arg(5)?;
        let samples =
            generate_training_samples(EXTERNAL_SPRT_POSITIONS, plies, seed, label_depth, schema)?;
        print!("{}", feature_schema_header(schema));
    ...
    if std::env::args().nth(1).as_deref() == Some("gate-file") {
        ...
        let path = std::env::args().nth(2).ok_or_else(|| {
            "usage: gate-file <net> <depth> <openings_file> [move_time_ms]".to_string()
        })?;
        ...
        let records =
            run_nnue_gauntlet_with_move_time(&fens, std::sync::Arc::new(net), config, move_time)?;
        let score = summarize(&records);
    ...
    if std::env::args().nth(1).as_deref() == Some("sprt") {
        ...
        eprintln!(
            ...
            score
                .elo_difference()
                .map_or_else(|| "n/a".to_string(), |elo| format!("{elo:.1}")),
        );
        return Ok(());
    }
...
fn stockfish_label() -> Result<(), String> {
    ...
    if args.len() != 6 && args.len() != 7 {
        return Err(
            "usage: stockfish-label <manifest> <split> <stockfish_config> <out_tsv> [schema]"
                .into(),
        );
    }
    ...
    let schema = if let Some(name) = args.get(6) {
        parse_feature_schema(name)?
    } else {
        FeatureSchema::RelativePieceSquareV1
    };
    ...
    if out.exists() {
        return Err(format!(
            "refusing to overwrite label output {}",
            out.display()
        ));
    }
...
fn manifest_fens(
    ...
    let index = match split {
        ...
        VALIDATION_SPLIT => 1,
        TEST_SPLIT => 2,
        _ => return Err(format!("unknown dataset split: {split}")),
    };
    let shard = manifest_path
        .parent()
        .ok_or_else(|| "manifest has no parent directory".to_string())?
        .join(format!("{split}.tsv"));
    let bytes = std::fs::read(&shard)
        .map_err(|error| format!("failed to read split {}: {error}", shard.display()))?;
    if sha256_hex(&bytes) != manifest.shard_sha256[index] {
        return Err(format!("split FEN hash does not match manifest: {split}"));
    }
    let text = std::str::from_utf8(&bytes)
        .map_err(|_| format!("split is not UTF-8: {}", shard.display()))?;
    if text.lines().next() != Some("fen\tsource") {
        return Err(format!("invalid split header: {split}"));
    }
    text.lines()
        .skip(1)
        .map(|line| {
            line.split_once('\t')
                .map(|(fen, _)| fen.to_string())
                .ok_or_else(|| format!("invalid split row: {line}"))
        })
        .collect()
}
...
fn write_stockfish_config(path: &Path, config: &StockfishConfig) -> Result<(), String> {
    if path.exists() {
        return Err(format!(
            "refusing to overwrite Stockfish config {}",
            path.display()
        ));
    }
    let body = format!(
...
fn read_stockfish_config(path: &Path) -> Result<StockfishConfig, String> {
    ...
    if lines.next() != Some("stockfish_config\t1") {
        return Err("unsupported Stockfish config format".into());
    }
    let mut values = BTreeMap::new();
    for line in lines {
        let (key, value) = line
            .split_once('\t')
            .ok_or_else(|| format!("invalid Stockfish config line: {line}"))?;
        if values.insert(key, value).is_some() {
            return Err(format!("duplicate Stockfish config field: {key}"));
        }
    }
    Ok(StockfishConfig {
        binary: values
            .remove("binary")
            .ok_or_else(|| "Stockfish config missing binary".to_string())?
            .into(),
        binary_sha256: values
            .remove("binary_sha256")
            .ok_or_else(|| "Stockfish config missing binary_sha256".to_string())?
            .to_string(),
        hash_mb: values
            .remove("hash_mb")
            .ok_or_else(|| "Stockfish config missing hash_mb".to_string())?
            .parse()
            .map_err(|_| "invalid hash_mb".to_string())?,
        node_budget: values
            .remove("node_budget")
            .ok_or_else(|| "Stockfish config missing node_budget".to_string())?
            .parse()
            .map_err(|_| "invalid node_budget".to_string())?,
        response_timeout: Duration::from_millis(
            values
                .remove("response_timeout_ms")
                .ok_or_else(|| "Stockfish config missing response_timeout_ms".to_string())?
                .parse()
                .map_err(|_| "invalid response_timeout_ms".to_string())?,
        ),
    })
}
...
fn dataset_build() -> Result<(), String> {
    ...
    let random_count = args[4]
        .parse::<usize>()
        .map_err(|_| "random_count must be an integer".to_string())?;
    let opening_count = args[5]
        .parse::<usize>()
        .map_err(|_| "opening_count must be an integer".to_string())?;
    let quiet_count = args[6]
        .parse::<usize>()
        .map_err(|_| "quiet_count must be an integer".to_string())?;
    let seed = args[7]
        .parse::<u64>()
    ...
    let total = random_count
        .saturating_add(opening_count)
        .saturating_add(quiet_count);
    ...
    if actual_source_counts != expected_source_counts
        || splits.values().map(Vec::len).sum::<usize>() != total
    {
        return Err(
            "dataset generation did not preserve the requested unique source composition".into(),
        );
    }
    ...
    for split in [TRAIN_SPLIT, VALIDATION_SPLIT, TEST_SPLIT] {
        let records = splits
            .get(split)
            .expect("all dataset splits are initialized");
...
fn reserve_output_directory(path: &Path) -> Result<(), String> {
    std::fs::create_dir(path).map_err(|error| {
        if error.kind() == std::io::ErrorKind::AlreadyExists {
            format!(
                "refusing to modify existing or reserved dataset output {}",
                path.display()
            )
        } else {
...
fn append_records(
    ...
    while records.len() < target_len {
        ...
        for fen in source_fens(
            source,
            remaining.saturating_mul(2).max(16),
            plies,
            seed.wrapping_add(batch),
        ) {
    ...
    for _ in 0..count.saturating_mul(4).max(16) {
        ...
        for _ in 0..plies.max(1) {
            ...
            let quiet: Vec<_> = moves
                ...
                .iter()
                .copied()
                .filter(|mv| board.piece_at(mv.to).is_none() && mv.promotion.is_none())
                .collect();
            if quiet.is_empty() {
                break;
            }
...
fn feature_schema_arg(index: usize) -> Result<FeatureSchema, String> {
    match std::env::args().nth(index) {
        Some(name) => parse_feature_schema(&name),
        None => Ok(FeatureSchema::RelativePieceSquareV1),
    }
}

fn parse_feature_schema(value: &str) -> Result<FeatureSchema, String> {
    if value == "v1" {
        return Ok(FeatureSchema::RelativePieceSquareV1);
    }
    let Some(buckets) = value.strip_prefix("halfka-v2-") else {
        return Err(format!("unsupported feature schema: {value}"));
    };
    let buckets = buckets
        .parse::<u8>()
        .map_err(|_| format!("invalid HalfKA bucket count: {buckets}"))?;
    if buckets == 0 || buckets != 64 {
        return Err(format!("unsupported HalfKA bucket count: {buckets}"));
    }
    Ok(FeatureSchema::HalfKaV2 { buckets })
}

fn feature_schema_name(schema: FeatureSchema) -> String {
    match schema {
        FeatureSchema::RelativePieceSquareV1 => "v1".to_string(),
        FeatureSchema::HalfKaV2 { buckets } => format!("halfka-v2-{buckets}"),
    }
}

fn feature_schema_header(schema: FeatureSchema) -> String {
    format!(
        "rfnn_tsv\t1\t{}\t{}\n",
        feature_schema_name(schema),
        schema.input_dimension()
    )
}
...
mod tests {
    ...
    fn output_directory_reservation_rejects_an_existing_path() {
        let path =
            std::env::temp_dir().join(format!("rusty-fish-reservation-{}", std::process::id()));
    ...
    fn label_fens_are_canonicalized_and_validated() {
        let canonical =
            canonical_label_fen("rnbqkbnr/pppppppp/8/8/8/8/PPPPPPPP/RNBQKBNR w KQkq - 00 01")
                .unwrap();
        assert_eq!(
            canonical,
            "rnbqkbnr/pppppppp/8/8/8/8/PPPPPPPP/RNBQKBNR w KQkq - 0 1"
    ...
    #[test]
    fn feature_schema_headers_identify_v1_and_halfka_dimensions() {
        let v1 = parse_feature_schema("v1").expect("v1 schema");
        assert_eq!(v1, FeatureSchema::RelativePieceSquareV1);
        assert_eq!(feature_schema_header(v1), "rfnn_tsv\t1\tv1\t768\n");

        let halfka = parse_feature_schema("halfka-v2-64").expect("HalfKA schema");
        assert_eq!(halfka, FeatureSchema::HalfKaV2 { buckets: 64 });
        assert_eq!(
            feature_schema_header(halfka),
            "rfnn_tsv\t1\thalfka-v2-64\t40960\n"
        );
    }

    #[test]
    fn feature_schema_parser_rejects_unknown_or_zero_bucket_schemas() {
        assert!(parse_feature_schema("halfka-v2-0").is_err());
